...
\end{adjustwidth}
	...
	quasiparticle ground state comprises the superfluid part. \blu{The latter is composed of a macroscopically-occupied condensate and correlated particle pairs due to s-wave interactions between constituent particles \cite{Bogolubov47,Vogels02}} 
	\blu{A consequence of these pairs is  %Bogoliubov's theory also predicts 
	that excited} single-particle modes %at large momentum $p$ 
	are populated even at zero temperature. % due to s-wave interactions between constituent particles.
	...
	Since the realization of atomic Bose-Einstein condensates (BECs) there has been considerable experimental \cite{Stewart10,Wild12,Chang16,Makotyn14,Eigen18,Xu06,Vogels02,pieczarka20,Lopes17_depletion,Cayla20,Kuhnle11,Sagi12,Fletcher17,Lopes17_quasiparticle,Mukherjee19,Carcy19} and 
	...
	To this end, we measure the momentum distribution of a helium condensate expanding from a harmonic trap. 
	...
	These factors result in an amplification of the momentum tails relative to the in situ values \blu{by a factor of up to about two}, and are not captured in the hydrodynamic approximation. 
	However, there remains a quantitative disagreement between simulation and the experiment \blu{because the experimental tails are even heavier by a large margin.}
...
\section{Background} 
	...
	\blu{particles \cite{Olshanii03,Decamp18,Chang16}} which decays as $\lim_{k\rightarrow\infty}\rho(\kvec)\propto k^{-4}$ \cite{PethickSmith,PitaevskiiStringari,Chang16}. 
	...
	% This observation is also in conflict with quantitative predictions of the tail amplitude drawn from the theory of contact interactions developed by Tan \cite{Tan08_energetics,Tan08_momentum,Tan08_virial}.
	...
	On the other hand, the amplitude of the tails was shown by Tan to be exactly the quantity called the \emph{contact} which is proportional to the derivative of the energy with respect to the s-wave scattering length \cite{Tan08_momentum, Braaten11}.
	...
	\rem{Note that hereon we refer to the momentum distribution $n(k)$ rather than the occupation numbers $\rho(k) = n(k) d^3k$, and that }%the prefactor ensures the wavefunction is normalized with respect to the integral (2\pi)^{-3} \int d^3 k n(k) = 1$.}
    \blu{the total number of atoms in this normalisation is $N=\frac{1}{(2\pi)^3}\int d^3 k\, n(k)$.}
	%a) We never talked about wavefunctions  b) as defined, rho is the real number of particles, so it should sum just to N. As far as I can tell, this stuff is not correct;y normalised.
...
\label{sec:analysis}
	...
	Note that for a non-interacting gas in the thermodynamic limit, the \blu{number of thermal atoms} %thermal fraction 
	...
	We account for this and the attendant twofold increase in the thermal fraction \blu{$\eta_T$}  (relative to the non-interacting case) by explicitly using $N_T$ as a fit parameter.
	%As $\eta_T$ is used to compute the condensed number (see below), we extract it directly from the time-of-flight profiles as described in section \ref{sec:th_spin}.
	\textcolor{red}{[[removed a sentence here, moved another]]} %\eta_T seemed to be off-topic.
	The thermal population decays super-exponentially with $k$, and hence cannot account for the counts we observe beyond $k\gtrsim 6~\micron^{-1}$. %this sentence moved down 
...
\label{sec:pow_issues}
	...
	Proceeding with a routine fit of the $k$-space histogram with an additional term of the form $C_\alpha/k^\alpha$ would appear to be an unobjectionable way to estimate the parameters of the purported quantum-depleted tail.
	...
	Hereafter we use $C_\alpha$ to refer to a fit parameter, as distinct from the total contact $\mathcal{C}$ computed using the sweep theorem (Eqn. (\ref{eqn:sweep_theorem})), and $C_\textrm{sim}$ as determined from our numerical simulations. \blu{All three have dimensions m${}^{-1}$ like $\mathcal{C}$.}
	
	If we augment the \blu{thermal} fit function (Eqn. (\ref{eqn:th_fun})) with a power-law term and leave $\alpha$ as a free parameter, the average exponent over all runs is 4.2(4). 
	...
	The red diamonds in Fig. \ref{fig:contact_determination_issues} (b) show the fit exponent and amplitude obtained from a fit (of the form $\blu{n(k)} %$\hat{n}(k) 
	...
	In particular if one assumes that the data conforms to a power law with $\alpha=4$, one is forced to conclude that the Tan theory is wrong \blu{for describing the observed high momentum tails}, but the inherent uncertainty in this approach precludes the possibility of such a definitive statement.
	

	Second, a deceptively reassuring result can be found by multiplying the empirical density by $k^4$, observing a flat region, and identifying this with \blu{a $k^{-4}$ tail.} %the quantum depletion. 
	...
	\footnote{Applying a fit of the form $\blu{f_{\rm fit}(k)=}n_T(k)k^4+C$ to data scaled as $\blu{f(k)} = k^4 n(k)$ minimizes the error term $\sum_i (k_{i}^{4}n(k_i)-k_{i}^4n_{fit}(k_i))$ which is equivalent to weighting the error terms \blu{for the original $n(k)$ data} by $k^4$. 
	Moreover, the fit is highly sensitive to the choice of weighting function.
	If one constrains the exponent in the fit \blu{of such an $f(k)$} at $\alpha=4$, then changing the \blu{power of the} \emph{weighting function} of the squared errors in the fit from \blu{$k^4$} by 0.1 \blu{to $k^{4\pm0.1}$} changes the fitted \blu{$C$} %$C_\alpha$  
	by a factor of ten.}
	...
	Following the rescaling operation $n'(k) = k^\alpha n(k)$, one has a \blu{function} %density 
	which would have the form $n'(k) \propto k^{-\delta}$. 
	Considering the range of $k$ examined in this (and the prior \cite{Chang16}) work, the scaled \blu{function} %density 
	...
	\rem{Moreover, the question of whether the data conforms to a power law at all is not amenable to a decisive conclusion.
	A log-normal distribution can also appear as straight lines over regions of a log-log plot and produce similarly accurate predictions as the power law \cite{virkar14,Clauset09}, as we discuss in the next section.
	This underscores the challenge of identifying power-law behaviour in range-limited data, because even with these parameters the log-normal distribution eventually diverges from the power law, albeit over far a far larger domain than available in either Helium experiment.}
	...
	In general, estimating the exponent of a purported power law is difficult and requires data spanning several orders of magnitude in scale \cite{Goldstein04,Clauset09,Virkar14,Hanel17}, which are not present in either \blu{of the} helium experiment\blu{s to date}.
	...
	\begin{center}
		\includegraphics[width=\columnwidth]{fig/QD/exp_results_plus}
			\caption{\blu{Analysis of Tan's tails.} A linear fit (a) shows that the product $N_0n_0$ is a good predictor of the number of counts within the region $(k_\textrm{min}=6~\micron^{-1},k_\textrm{max}~\micron^{-1})$, consistent with Eqn (\ref{eqn:pred_scaling}) 
			(solid orange line, dashed lines 95\% CI). The gradient \blu{$\Lambda$} %$\Lambda_\textrm{fit}$ 
			\blu{in Eqn. (\ref{eqn:Lambda})} can be predicted using Eqn (\ref{eqn:pred_scaling}) (\blu{$\Lambda_{\rm pred}$} solid purple line) but this disagrees with the experiment by a factor of about 8. Our simulations (dashed line) show an increase in counts after release but by less than in the experiment. 
			% As shown in (b,c), $\Lambda$ depends on the choice of $k$ bounds (with $k_\textrm{max}$ and $k_\textrm{min}$ fixed, respectively).
			In (b,c) we show predictions of $\Lambda$ based on Eqn (\ref{eqn:pred_scaling}) (\blu{$\Lambda_{\rm pred}$}, blue), along with the predictions \blu{from Eqn. (\ref{eqn:Lambda}) using} a density function $n(k)\blu{=\mathcal{AC}/k^4}$ that \blu{has an additional prefactor $\mathcal{A}=$8(3)} %times Eqn. (\ref{eqn:pred_scaling})
			(green) and one that has %exactly the amplitude $\mathcal{C}$ but 
			a modified exponent of \blu{$\alpha=3.86(2)$ via $n(k)=\mathcal{C}/k^{\alpha}$} (yellow). 
			The \blu{experimental} data \blu{$\Lambda_{\rm fit}$} (points) cannot distinguish between either of these two alternative hypotheses, and therefore provides limited information about \blu{the prefactor $\mathcal{A}$ or exponent }% $C$ or
			$\alpha$.
			...
			\rem{A log-normal distribution produces nearly identical predictions (red, offset vertically for visibility).}
			}
...
\label{sec:good_test}
	...
	\begin{equation}
		...
		\label{eqn:pred_num}
	\end{equation}
	For fixed $k_\textrm{min}$ and $k_\textrm{max}$, Eqn. (\ref{eqn:pred_num}) has the form \blu{
	\begin{equation}
...
(c.f. Eqn. \ref{eqn:pred_scaling}). 
	...
	As above, we choose $k_\textrm{min}=6~\micron^{-1}$ which lies outside the thermal part for all our data sets (see Fig. \ref{fig:contact_determination_issues} a).
...
%     so that would be approx 0.8(3) times Tan theor
	...
	A linear fit of the form $\hat{N}_{k_\textrm{min},k_\textrm{max}} = \Lambda_\textrm{fit} n_0 N_0 + \beta$ yields \blu{an intercept} %a vertical intercept $c$ 
	...
	variables \blu{$N_{k_{\rm min},k_{\rm max}}$} %$N_\textrm{exp}$ 
	...
	For comparison, a linear fit proves that the atom number \blu{$N$} itself is a poor predictor of the detected number ($r^2=0.05~,p=0.54$), as is the density \blu{$n_0$} alone ($r^2=0.4~,p=0.04$).
	...
	Ultimately, \blu{it turns out that} such a distinction between these hypotheses is not possible given the data at hand. 
	...
	\rem{In Fig. \ref{fig:exp_results} (b,c) show, in red, predictions obtained by assuming log-normally distributed $k$ with parameters $(\mu,\sigma) \approx (1.235,0.95)$ and normalized to the relevant amplitude.
	...
	\rem{Ultimately, as we detail in the Discussion (section \ref {sec:discussion}), the most prudent way through this dilemma is via alternative experimental designs.}
	
	
	One conclusion remains robust: There are \blu{almost} about ten times as many detections in the depletion region as one would expect based on the Tan theory \blu{\emph{in situ}}. 
...
\subsubsection{Trap configuration}
	...
	\rem{The dominant uncertainty in the collection efficiency $\epsilon$ is the 25\% error in the detector quantum efficiency (QE), whereas the other factors (cutoff angle $\phi_c$ and transfer efficiency $\eta_0$) are more precisely known.
	We performed the analysis described above for a range of $\phi_c$ and values of the QE and found that the excess of counts (expressed as $\Lambda_\textrm{fit}/\Lambda_\textrm{pred}$) was not significantly affected, which we summarize in Table \ref{tab:choice_indep}.}
...
\label{sec:spinpop}
	...
	\rem{Such counts constitute about 10(5)\% of the detection events in the ROI. 
	The calibration without the Landau-Zener transfer enables us to correct for their contribution by subtracting the counts measured in the calibration runs from the total used in Sec \ref{sec:good_test}.
	% Generally, this contribution is much smaller than the discrepancy described reported above, and . 
...
\label{STAB}
	\begin{figure}
	        ...
	        \label{fig:sim_fig}
	\end{figure}
	In order to \blu{understand} whether any depletion could survive the expansion and to investigate what effects are taking place during the initial release, we performed simulations of the BEC expansion from harmonic traps using the first principles STAB method \cite{Deuar11,Kheruntsyan12}. 
	...
	Fig. \ref{fig:sim_fig} shows the time evolution of the tail amplitude \blu{$C_{\rm sim}$ extracted from a $n(k)=C_{\rm sim}/k^4$ fit to the simulated density} %for the simulated traps 
	using \blu{an ROI with the same cutoff elevation angle $\phi_c=\pi/3$ as in the experiment.}  
	...
	\blu{An analysis of the occupation of the tails according to (\ref{eqn:pred_num}), gives very similar factors $\mathcal{A}_{\rm sim}$ for the increase in the strength of the tails (relative to \emph{in-situ} predictions) during evolution, as shown in Table~\ref{tab:Nminmax}.}

	To understand the disagreement with earlier theory \cite{Qu16}, which predicted no depletion survival, we also investigated the effect of adiabatic expansion on the in-trap depletion by simulating a slow decrease of the transverse trapping frequencies by a factor of two (CS), and found that the in-trap contact \blu{$C_{\rm sim}$ as well as the the tail strength $N_{k_{\rm min},k_{\rm max}}$} decreased roughly as predicted by Eqn. (\ref{eqn:pred_scaling}) in these instances --- see the dashed line in Fig.~\ref{fig:sim_fig} (b) \blu{and Table~\ref{tab:Nminmax}.} 
	We also found that the relative factors between Eqn. (\ref{eqn:pred_num}) and the simulated tails (i.e. $C_\textrm{sim}/\mathcal{C}$ \blu{and $\mathcal{A}_{\rm sim}$) depend} on choice of cutoff angle $\phi_c$. 
	...
	\rem{We find that $C_\textrm{sim}$ \blu{and $\mathcal{A}_\textrm{sim}$ are} larger for smaller collection regions that are more tightly concentrated about the %vertical 
	(strong) trapping axis, whereas larger collection angles (including areas closer to the weak axis) produce a lower $C_\textrm{sim}$. 
	\blu{This is shown in Table~\ref{tab:Nminmax} while also }
	%That is, 
	with $\phi_c$ values of 60, 70, and 80 degrees, %(in line with Tab. \ref{tab:choice_indep}),
	$C_\textrm{sim}$ was 1.6(1), 1.9(2), and 2.2(3) times $\mathcal{C}$, respectively.
	...
	Details regarding the implementation of the simulations can be found in \todo{Cite paper}
% \begin{widetext}
...
\label{sec:discussion}
	...
	We surmise that the tails are indeed a signature of quantum depletion (per the first point), albeit subject to some physical effect during the expansion as discussed below, \blu{or some nonequilibrium enhancement in the trapped state}.
	%well we do mention vague possibilities in-trap, so it glares a bit to exclude them here and not later.
	...
	This also presents as an increase in $C_\textrm{sim}$ \blu{and $\mathcal{A}_{\rm sim}$ for} collection regions \blu{with a higher $\phi_c$}. 
	...
	This \blu{also corroborates the above interpretation} %constitutes a partial explanation 
	...
	Quasiparticles whose wavenumber exceeds that of the speed of sound lie within the particle-like branch of the Bogoliubov spectrum, which for our considerations is on the order of $k\gtrsim2~\micron^{-1}$, well within the thermal velocities. \blu{Therefore the phonon/particle changeover is not responsible for the inflections seen at much higher $k$ in Fig.~\ref{fig:contact_determination_issues}(a).} 
	...
	We thus find a partial explanation for the deviation of the far-field distribution from both the predicted in-situ depletion and the hydrodynamic reabsorption: The interplay between coherent absorption of Bogoliubov excitations and the dispersal of the \blu{interaction energy} %chemical potential 
...
% % 	\begin{figure}[t]
% % 	    \includegraphics[width=0.4\textwidth]{fig/QD/main/exp_cartoon}
...
% % 	\begin{figure}[t]
% % 	        \includegraphics[width=0.5\textwidth]{fig/QD/main/contact_determination_rev}
...
% % 	\begin{figure}[t]
% % 	        \includegraphics[width=0.5\textwidth]{fig/QD/main/results_comp}
...
% % \begin{figure*}[!t]
% % 	\begin{center}
% % 		\includegraphics[width=\textwidth]{fig/QD/main/frac_cal_profile}
...
% % % 	\begin{figure}[!h]
% % % 	\begin{center}
% % % 		\includegraphics[width=0.8\textwidth]{fig/QD/main/m1_frac_anal}
...
% %     \begin{figure}[!b]
% % 	\begin{center}
% % 		\includegraphics[width=\columnwidth]{fig/QD/main/spinpop}
...
% % 	% \begin{figure}[!h]
% % 	% \begin{center}
% % 	% 	\includegraphics[width=\textwidth]{fig/QD/main/profile_overlay}
...
% % % \begin{figure}
% % % 	\centering
% % % 	\includegraphics[width=0.5\textwidth]{fig/QD/main/counts_per_run}
...
% % 	\begin{figure}[t]
% % 	        \includegraphics[width=0.5\textwidth]{fig/QD/main/time_dep_theory}
