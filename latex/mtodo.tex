...
\begin{itemize}
\item {Clarify/check logic}
\item {Consider better separating atomic structure and atomic interaction if possible, maybe just some extra headings}
\item {orientation dependence goes here}
\item {Add citations for dipole trap history}
\item {Aren't the numbers for quantum states J, $m_J$ for helium?  Just make sure it's consistent between the intro and the spectroscopy chapter.}
\item {Flesh out the below and/or move modern calculations to spectroscopy section, add citations as appropriate}
\item {Might make more sense after BEC?}
\item {why is $a_{11}
\item {Braun13}
\item {this sentence doesn't add much}
\item {should this be an approx sign rather than a proportional sign?  Doesn't really make sense to me what you are saying as written}
