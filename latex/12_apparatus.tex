...
% \hline
%    & Detuning (MHz) &   Detuning (FWHM)  &  Intensity (mW/mm$^2$)  &  Intensity ($I_{sat}$) \\
%  \hline\hline
%     Coll & 247.91 & 154.94 & 0.028125 & 0.16841\\
%     Bend & 247.91 & 154.94 & 0.049546 & 0.29668\\
%     ZS & 239.4 & 149.62 & 0.061978 & 0.37113\\
%     MOT 1 (H) & 112.98 & 70.612 & 0.023846 & 0.14279\\
%     MOT 1 (V) & 112.98 & 70.612 & 0.11382 & 0.68153\\
%     Focus & 124.43 & 77.769 & 0.00024373 & 0.0014595\\
%     MOT 2 (H) & 111.69 & 69.806 & 0.0032331 & 0.01936\\
%     MOT 2 (V) & 111.2 & 69.5 & 0.10207 & 0.61118\\
% \end{tabular}
...
\hline
\end{tabular}
\caption{Beam parameters for the BiQUIC machine. \todo{These are wrong, will fix}}
\label{tab:biquic_beams}
\end{table}
...
\subsection*{Beam forming}
	...
	This is followed by a `bending' stage wherein a single beam, picked off from the collimator, deflects the metastable atoms by a few degrees through a second stage of differential pumping, which blocks the line of sight from the source to the MOT, into the Zeeman slower chamber.
...
\subsection*{The Science Chamber}
	...
	A comphrehensive introduction to evaporative cooling can be found in \ref{ketterle96}; see \cite{FootAtomic,PethickSmith} for more compact discussions. 
...
\label{sec:spec_laser}
	...
	\rem{After exiting the fibre, the light was linearly polarized and a small fraction was directed onto a photodiode, whose voltage served as the input parameter to a PID loop.
	The control loop had a 3dB bandwidth of 170kHz and was able to stabilize the beam power to within a relative error of 0.3\%.}
...
\subsection*{Single-atom detection with the MCP-DLD stack}
	...
	When an atom strikes the surface of a pore after falling from the trap, a second-order process releases a free electron with most of the 19.8eV as kinetic energy \cite{Jagutzi02,Hotop96}.
...
\subsection{Laser lock control protocol}
	...
	\rem{The software feedback system returns commands to the laser at a rate of $\sim 14$~Hz. The system has sufficient bandwidth to achieve a $\sim900$~ms rise time (10\% to 90\%) for a $\sim300$~MHz frequency step, and during interrogation we measure a typical (in-loop) standard deviation of 170~kHz. The set point in this control loop is stepped every two BEC cycles (one interrogation and one calibration) to scan out the measurements described in chapters X and Y. The stabilization of the laser frequency uses measurements of the red side of the laser system (predoubling cavity) such that interruptions to the doubling cavity do not impact the optical frequency feedback.}
